...
|\   ____\|\   ___ \|\___   ___\     |\   __  \|\   __  \|\___   ___\ 
\ \  \___|\ \  \_|\ \|___ \  \_|     \ \  \|\ /\ \  \|\  \|___ \  \_| 
 \ \_____  \ \  \ \\ \   \ \  \       \ \   __  \ \  \\\  \   \ \  \  
  \|____|\  \ \  \_\\ \   \ \  \       \ \  \|\  \ \  \\\  \   \ \  \ 
    ____\_\  \ \_______\   \ \__\       \ \_______\ \_______\   \ \__\
   |\_________\|_______|    \|__|        \|_______|\|_______|    \|__|
   \|_________|                                                     
   """
...
async def crypto_payment_callback(update: Update, context: CallbackContext):
    try:
        # choose payment net
        #crypto = AioCryptoPay(token=config.CRYPTO_TOKEN_PROD, network=Networks.MAIN_NET)
...
def main():
    try:
        ...
        application = Application.builder().token(config.BOT_TOKEN_PROD).concurrent_updates(True).build() # .post_stop(post_stop)
